...
const useWebSocketStore = create((set, get) => ({
    ...
    connect: () => {
        ...
        const socket = new SockJS(`${protocol}//${window.location.host}/silverorder/api/ws-stomp`);

        console.log(`${protocol}//${window.location.host}/silverorder/api/ws-stomp`);
    ...
    subscribeToOrder: (orderId) => {
        ...
        if (orderId && get().stompClient && get().stompClient.connected) {
            get().stompClient.subscribe(`/topic/orderStatus/${orderId}`, (message) => {
                try {
            ...
            console.log(`지금부터 ${orderId}번 주문의 상태를 알림으로 확인할 수 있습니다.`); // 구독 성공 로그 출력
        } else {
